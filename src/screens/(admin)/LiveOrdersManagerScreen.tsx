...
export default function LiveOrdersManagerScreen({ route }: Props) {
  ...
  // Set of order IDs currently being updated — buttons disabled while in-flight
  ...
  async function handleTriggerTestOrder() {
    ...
    if (testAlarmActive) {
      // Second tap — stop the looping alarm
      await stopLoopingAlarm();
      setTestAlarmActive(false);
    } else {
      // First tap — start looping alarm
  ...
  async function handleUpdateStatus(orderId: number, newStatus: 'CONFIRMED' | 'COMPLETED' | 'REJECTED' | 'CANCELLED') {
    ...
    try {
      ...
      if (soundEnabled) playStatusChangeSound();
    } catch (err: unknown) {
      ...
      // Re-fetch to restore the true server state (silently — no spinner)
      fetchOrders(false, true);
    } finally {
        ...
        <View className="mt-1">
          ...
          {isPending ? (
            <View className="flex-row gap-2.5">
              <TouchableOpacity
                className={cn('flex-1 rounded-lg py-2.5 items-center', isProcessing ? 'opacity-50' : '', oledDark ? 'bg-red-950' : 'bg-red-100')}
              ...
              <TouchableOpacity
                className={cn('flex-1 rounded-lg py-2.5 items-center bg-emerald-600', isProcessing ? 'opacity-50' : '')}
            ...
            <View className="flex-row gap-2.5">
              <TouchableOpacity
                className={cn('self-end', isProcessing ? 'opacity-50' : '')}
    ...
    <View className={cn('flex-1', oledDark ? 'bg-[#09090B]' : 'bg-gray-100')}>
      ...
      <View className={cn('px-5 py-4 border-b items-center', oledDark ? 'bg-[#09090B] border-[#1F1F23]' : 'bg-white border-gray-200')}>
        ...
        <View className="flex-row items-center justify-center gap-2 mt-3.5 flex-wrap">
          ...
          >
            {testAlarmActive ? (
              <ZapOff size={13} color={oledDark ? '#F87171' : '#DC2626'} strokeWidth={2.2} />
            ) : (
              <Zap size={13} color={oledDark ? '#34D399' : '#059669'} strokeWidth={2.2} />
            )}
            <Text className={cn('text-xs font-bold', testAlarmActive ? (oledDark ? 'text-red-300' : 'text-red-600') : (oledDark ? 'text-emerald-300' : 'text-emerald-700'))}>
